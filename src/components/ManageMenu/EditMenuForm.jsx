...
const EditMenuForm = () => {
  // const editmenuData = JSON.parse(localStorage.getItem("editStaff"));
  ...
  const handleSubmit = (e) => {
    ...
    axios
      ...
      .then((response) => {
        swal("Thanks for creating menu!", "success").then((value) => {
          setParams(fields);
        });
      })
      .catch((error) => {
      ...
      <div className="h-[70vh] w-11/12  rounded-lg bg-white  flex flex-col">
        ...
        <div className="flex flex-col w-full h-2/6">
          ...
          <div className=" h-1/3 w-full mt-2 flex justify-center items-center">
            <Button text="Update Menu" onClick={handleSubmit} className="pl-8 pr-8"></Button>
          </div>
        </div>
      </div>
    </form>
